...
def get_car_location():
    print("[DEBUG] Triggering V7 location request")

    # ESP32 will answer V8 with either:
    #   {"source":"onstar","lat":...,"lng":...}  (preferred)
    # or legacy Wi-Fi scan payload:
    #   {"wifiAccessPoints":[...]}
    blynk_pulse("V7")

    payload = None
    for i in range(15):
        print(f"[DEBUG] Waiting for V8 location data... attempt {i+1}")
        payload = blynk_get("V8")
        print(f"[DEBUG] V8 value: {payload}")
        if payload:
            break
        time.sleep(1)
    else:
        return jsonify({"error": "No location data from V8"}), 400

    if isinstance(payload, str):
        try:
            payload = json.loads(payload)
        except Exception:
            return jsonify({"error": "Invalid location data in V8"}), 400

    if isinstance(payload, dict) and coords_look_sane(payload.get("lat"), payload.get("lng")):
        loc = build_map_result(
            payload.get("lat"),
            payload.get("lng"),
            payload.get("accuracy", 10),
            payload.get("source", "onstar"),
            {
                "speedKmh": payload.get("speedKmh", 0),
                "heading": payload.get("heading", 0),
                "ageSec": payload.get("ageSec", 0),
            }
        )
        print(f"[DEBUG] Direct vehicle location result: {loc}")
        return jsonify(loc)

    if not isinstance(payload, dict) or "wifiAccessPoints" not in payload:
        return jsonify({"error": "V8 did not contain OnStar coords or Wi-Fi scan data"}), 400

    loc = google_locate(payload)
    if not loc:
        return jsonify({"error": "Google geolocation failed"}), 500

    print(f"[DEBUG] Wi-Fi geolocation result: {loc}")
    return jsonify(loc)
